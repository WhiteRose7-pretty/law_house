...
class UserQuestionKnown extends Model
{
    /**
     ...
     * @var string
     */
    protected $table = 'users_questions_known';
    ...
    static public function available_ids($user_id, $questions_set_ids, $limit=0, $law_ids=null, $set_date=null)
    {
        ...
        if($law_ids && $set_date){
            $tmp = UserQuestionKnown::with(['laws'])->select('id', 'user_id')->where('user_id', $user_id)->whereIn('questions_set_id', $questions_set_ids)
            ...
            $tmp = UserQuestionKnown::with(['laws'])->select('id')->where('user_id', $user_id)->whereIn('questions_set_id', $questions_set_ids);
        }
