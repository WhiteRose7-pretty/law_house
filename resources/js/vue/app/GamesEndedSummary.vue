                        ...
                        <div class="pie-chart col-12 col-md-3 text-center p-1  yh-v-o-70">
                            <svg class="my-pie" viewBox="0 0 200 200">
                                ...
                                <circle r="85" cx="100" cy="100" fill="transparent"
                                 ...
                                 />
                                 <text x="100" y="105" dominant-baseline="middle" text-anchor="middle" fill="#666" font-size="3em" font-weight="500">{{Math.round(test.questions_answered_correct_count/test.user_game.questions_count*100)}}%</text>
                            </svg>
                        </div>
                        <div class="col-12 col-md-9 pt-2 pl-5">
                            <h4 class="float-md-right yh-v-o-60 font-italic">{{dateF(test.user_game.finished_at)}}</h4>
                            <h2 class="yh-fw-5 mb-4">{{test.user_game.name}}</h2>
                            <h4 class="mb-2">{{test.user_game.info}}</h4>
                            ...
                            <p style="font-size:1.4rem;">
                                Wynik: {{test.questions_answered_correct_count}}/{{test.user_game.questions_count}}
                                <span v-if="test.user_game.questions_count - test.questions_answered_count>0"><br>Pominiętych: {{test.user_game.questions_count - test.questions_answered_count}}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="float-left yh-v-o-70">
                    ...
                    <span v-if="index==test.questions.length">
                        <span class="btn btn-outline-secondary btn-sm" v-on:click="scrollFirst('all')">
                            Wszystkie <span class="d-none d-sm-inline-block">odpowiedzi</span>
                        </span>
        ...
        fetch(id,filter='noquestions') {
            ...
            axios
                ...
                .then(response => {
                    ...
                    this.test = JSON.parse(JSON.stringify(response.data.game_participant));
                    this.index=0;
                    this.loading = false;
                }).catch(error => {
                    this.loading = false;
