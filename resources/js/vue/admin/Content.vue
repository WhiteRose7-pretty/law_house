    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="p-3 mb-4" style="background-color: #f1f1f1;">
                ...
                <router-link v-if="current_category!='info'" :to="{ name: 'content.edit', params: { category: current_category, id: 0} }" class="btn btn-success btn-sm text-white float-right">Utwórz nowy</router-link>
            </div>
            ...
            <table v-if="pages > 0" class="table">
                ...
                <tbody>
                    <tr v-for="{id,title,created_at,updated_at,published_at} in list" v-bind:data-id="id" class="content-row">
                        <th scope="row">{{id}}</th>
                        <td>{{title}}</td>
                        <td><span v-if="updated_at">{{dateF(updated_at)}}</span><span v-else>{{dateF(created_at)}}</span></td>
